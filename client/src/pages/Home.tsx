...
const explorerUrl = "https://explorer-studio.genlayer.com/address/0xec5BB6E6f7B950914d55D34d931e0032935c8e89";
...
const pipeline = [
  { id: "01", label: "Sources", title: "Bounded public evidence", copy: "2–8 HTTPS sources are rendered independently. Source text is treated as untrusted evidence, never as instructions.", icon: Network, color: "cyan" },
  { id: "02", label: "Adjudication", title: "Structured judgment", copy: "Each validator assesses the same proposition and returns a constrained JSON result instead of free-form prose.", icon: Sparkles, color: "lime" },
  { id: "03", label: "Equivalence", title: "Meaning over wording", copy: "Consensus checks status, source coverage, confidence bounds, and rationale length—without demanding identical LLM language.", icon: ShieldCheck, color: "coral" },
  { id: "04", label: "State", title: "Auditable attestation", copy: "The result, source snapshot, timestamp, and challenge state become durable contract records.", icon: FileCheck2, color: "paper" },
];

const states = {
  evidence: { eyebrow: "01 / source set", title: "Public evidence enters the room.", copy: "EvidenceQuorum accepts a proposition and a bounded set of source URLs. The contract keeps the URLs attached to the final record so downstream builders can inspect what was assessed.", metric: "02—08", metricLabel: "HTTPS SOURCES" },
  consensus: { eyebrow: "02 / validator agreement", title: "Consensus is a validity boundary.", copy: "Validators do not need to phrase their rationales the same way. They do need to agree on a valid state: supported, refuted, or inconclusive—with evidence coverage that clears the threshold.", metric: "4 / 4", metricLabel: "REQUIRED FIELDS" },
  record: { eyebrow: "03 / durable state", title: "A claim becomes inspectable state.", copy: "The contract appends an Attestation record and its source snapshot. A challenge marks the record without erasing history, giving applications a conservative audit trail.", metric: "13", metricLabel: "OFFLINE TESTS PASSED" },
};
